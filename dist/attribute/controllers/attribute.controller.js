...
class AttributeController {
    ...
    async removeMany(req, res) {
        try {
            ...
            const result = await attribute_model_1.Attribute.deleteMany({ _id: { $in: ids } });
     ...
     */
    async removeManyValues(req, res) {
        try {
            const { idOrCode } = req.params;
            ...
            doc.values = doc.values.filter((v) => !valueIds.includes(String(v._id)));
